...
export default function PipelineView({ expId }) {
  ...
  return (
    <div className="border border-zinc-800 rounded-lg p-4 overflow-x-auto">
      ...
      <svg width={svgW} height={svgH} style={{ fontFamily: "'JetBrains Mono',monospace" }}>
        ...
        {plates.map((plate) => {
          ...
          if (!pos) return null;
          const tc = typeColors[plate.type] || typeColors.source;
          ...
          return (
            <g key={plate.id} onClick={() => handleClick(plate.id)} style={{ cursor: "pointer" }}>
              ...
              <text x={pos.x + nodeW / 2} y={pos.y + 20}
                textAnchor="middle" fill="#e4e4e7" fontSize={11} fontWeight="bold">
